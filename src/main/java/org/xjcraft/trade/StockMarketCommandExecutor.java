...
public class StockMarketCommandExecutor implements CommandExecutor {
    ...
    private void excuteLists(CommandSender commandSender, Command command, String s, String[] strings) {
        List<CustomItem> customs = Dao.getEbeanServer().find(CustomItem.class).findList();
    ...
    private void excuteGet(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == 2) {
            try {
                CustomItem custom = Dao.getEbeanServer().find(CustomItem.class).where().ieq("name", strings[1]).findUnique();
...
//                    commandSender.sendMessage(item.toString());
    ...
    private void excuteAdd(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == 2) {
            ...
            List<CustomItem> c = Dao.getEbeanServer().find(CustomItem.class).where().ieq("name", strings[1]).findList();
            List<CustomItem> c2 = Dao.getEbeanServer().find(CustomItem.class).where().ieq("flat_item", flatItem).findList();
            ...
            if (c.size() != 0 || c2.size() != 0) {
                ...
                try {
                    Dao.getEbeanServer().save(custom);
    ...
    private void excuteHand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        try {
            ...
            temp.setAmount(1);
            CustomItem customItem = Dao.getEbeanServer().find(CustomItem.class).where().ieq("flatItem", SerializeUtil.serialization(temp)).findUnique();
            if (customItem != null)
    ...
    private void excutePrice(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings.length == 1) {
            ...
            try {
                ...
                try {
                    CustomItem customItem = Dao.getEbeanServer().find(CustomItem.class).where().ieq("name", materialInput).findUnique();
                    if (customItem == null)
...
//                    e1.printStackTrace();
    ...
    private void excuteList(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        List<Trade> Tradelist = Dao.getEbeanServer().find(Trade.class).setDistinct(true).where().orderBy().asc("material").findList();
    ...
    private void sendDetail(CommandSender commandSender, ItemStack itemStack) {
        ...
        // FIXME: 2016/3/25
        List<History> list = Dao.getEbeanServer().find(History.class).where().ieq("material", material.name()).ieq("durability", durability + "").orderBy().desc("id").findList();
        try {
            CustomItem customItem = Dao.getEbeanServer().find(CustomItem.class).where().ieq("flat_item", SerializeUtil.serialization(itemStack)).findUnique();
            if (customItem != null) {
                title = customItem.getName();
                list = Dao.getEbeanServer().find(History.class).where().ieq("material", "S:" + customItem.getName()).ieq("durability", "0").orderBy().desc("id").findList();
            }
